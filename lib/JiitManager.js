...
// Use Next.js API route for proxy - no separate backend needed
// In production (Vercel), this will be the same domain
// The jsjiit library appends /proxy/{path} to this URL
const PROXY_URL = typeof window !== 'undefined'
    ? window.location.origin + '/api'  // Browser: use current origin
    : process.env.NEXT_PUBLIC_SITE_URL + '/api' || 'http://localhost:3000/api';  // Server: use env or localhost
...
class PortalEngine {
        ...
        try {
            ...
            registrations.subjects.forEach((raw) => {
                ...
                if (subjectsMap.has(code)) {
                    // Merge components for duplicate subject codes
                    ...
                    subjectsMap.set(code, {
                        ...
                        components: components.length > 0 ? components : ['L']
                    });
                }
            });
    ...
    async getAttendanceOverview(semester) {
        ...
        try {
            ...
            const overview = rawData.studentattendancelist.map((raw) => {
                ...
                // Use individualsubjectcode (e.g., "15B11CI514") to match subjects
                // Full subjectcode format is like "ARTIFICIAL INTELLIGENCE(15B11CI514)"
                const subjectCode = raw.individualsubjectcode || raw.subject_code || 'UNKNOWN';

                // Extract subject name from the full format "SUBJECT NAME(CODE)"
                const fullCode = raw.subjectcode || '';
                const match = fullCode.match(/^(.+?)\s*\(/);
                const subjectName = match ? match[1] : raw.subjectdesc || 'Unknown Subject';
                ...
                return {
                    ...
                    subjectName,
                    percentage,
                    // Component-level percentages for detailed display
                    Lpercentage: raw.Lpercentage !== undefined ? parseFloat(raw.Lpercentage) : undefined,
                    Tpercentage: raw.Tpercentage !== undefined ? parseFloat(raw.Tpercentage) : undefined,
                    // Present/Total for L+T combined
                    LTpresent: raw.LTpresent !== undefined ? parseInt(raw.LTpresent) : undefined,
                    LTtotal: raw.LTtotalclasses !== undefined ? parseInt(raw.LTtotalclasses) : undefined,
                    // Store internals for daily attendance fetch
                    _subjectId: raw.subjectid,
                    _componentIds: componentIds,
                    // Keep individualsubjectcode for daily attendance API
                    individualsubjectcode: raw.individualsubjectcode
                };
            });
    ...
    async getSubjectDailyAttendance(semester, subjectId, subjectCode, componentIds) {
        ...
        try {
            ...
            const response = await this.portal.get_subject_daily_attendance(
                this.rawSemester,
                subjectId,
                subjectCode,
                componentIds
            ...
            // The API returns { studentAttdsummarylist: [...] }
            // Extract the array from the response
            const daily = response?.studentAttdsummarylist || response || [];

            this.log('Daily attendance fetched', { count: Array.isArray(daily) ? daily.length : 'N/A' });
            return daily;
