...
import { parseDailyAttendanceEntry } from './parser';
...
export function normalizePastSessions(dailyData) {
    ...
    return dailyData
        .map(record => {
            const parsed = parseDailyAttendanceEntry(record);
            if (!parsed) return null;

            const date = parseERPDate(parsed.date);
            if (!date) return null;

            const status = parsed.isPresent ? 'present' : 'absent';

            return {
                date,
                dateKey: `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`,
                time: parsed.date?.match(/\((.+)\)/)?.[1] || '',
